...
def load_dataset(dataset_dir: str, use_augmentation: bool = True):
    """
    Loads dataset where each image filename = student name.
    ...
    if not image_files:
        log.error(f"No images found in {dataset_dir}")
        return None, None, None

    log.info(f"Found {len(image_files)} face images")
    ...
    for img_file in image_files:
        ...
        if student_name not in label_lookup:
            label_lookup[student_name] = label_id
            label_map[label_id] = student_name
            label_id += 1

        current_label = label_lookup[student_name]
